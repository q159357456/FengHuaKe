...
-(void)rightButton
{
    UIButton *backButton = [[UIButton alloc] initWithFrame:CGRectMake(0,0,35,17)];
    backButton.titleLabel.font=[UIFont systemFontOfSize:15];
    backButton.accessibilityIdentifier = @"ddFriends";
    [backButton setTitle:@"发表"  forState:UIControlStateNormal];
    [backButton addTarget:self action:@selector(publish) forControlEvents:UIControlEventTouchUpInside];
...
-(void)publish
{
    NSString *title = [NSString stringWithFormat:@"%@", self.textView.titleTextField.text];
    NSString *content=[self exportHTML];
    if (title.length==0) {
    ...
    if (content.length==0) {
       ...
       NSLog(@"exportHTML:%@",[self exportHTML]);
    [self netPubishWithTitle:title Content:content];
 
    
}
#pragma mark - net
-(void)upLoadPicWithNSTextAttachment:(NSTextAttachment*)textAttachment Data:(NSData*)imageData
{
    NSArray *array=@[imageData];
    DefineWeakSelf;
    [CatageManagerVM NewImageDataArray:array Success:^(id responseData) {
        NSDictionary *dic=(NSDictionary*)responseData;
        NSLog(@"dic--%@",dic);
     
        if ([dic[@"sysmodel"][@"blresult"] intValue]) {
            NSString *url=dic[@"sysmodel"][@"strresult"];
        
            textAttachment.attachmentType = LMTextAttachmentTypeImage;
            textAttachment.userInfo = [DataProcess PicAdress:url];
            
        }else
        {
            [weakSelf showHint:@"图片上传失败，请检查网络"];
...
-(void)netPubishWithTitle:(NSString*)title Content:(NSString*)content
{
    // DataList.title：标题；DataList.tips：标签；DataList.content：内容；DataList.createuser：会员ID；DataList.fromdate：公布时间
    NSString *dataList=[DataProcess getJsonStrWithObj:@[@{@"title":title,@"tips":@"ios",@"content":content,@"createuser":UniqUserID,@"fromdate":[DataProcess getCurrrntDate]}]];
    DefineWeakSelf;
    [CatageManagerVM NewAddNotesDataList:dataList Success:^(id responseData) {
        NSDictionary *dic=(NSDictionary*)responseData;
        NSLog(@"dic--%@",dic);
...
//        }
        [self upLoadPicWithNSTextAttachment:attachment Data:degradedImageData];
    });
}
