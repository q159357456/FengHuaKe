...
{
    NSLog(@"%@",error);
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"注册APNS失败"
...
- (void)loginStateChange:(NSNotification *)notification
{
    ...
    GBNavigationController *navigationController=nil;
    if (loginSuccess) {//登陆成功加载主窗口控制器
        NSLog(@"登陆成功");
        [[ApplyViewController shareController] loadDataSourceFromLocalDB];
        if (!self.mainTabBarController) {
            self.mainTabBarController=[[MainTabBarController alloc]init];
            navigationController=[[GBNavigationController alloc]initWithRootViewController:self.mainTabBarController];
        }else
        {
            navigationController=(GBNavigationController*)self.mainTabBarController.navigationController;
        }
        ...
        if (self.mainTabBarController) {
            [self.mainTabBarController.navigationController popToRootViewControllerAnimated:NO];
        }
        self.mainTabBarController=nil;
        [ChatDemoHelper shareHelper].mainVC = nil;
        LoginViewController *loginController = [[LoginViewController alloc] init];
        navigationController = [[GBNavigationController alloc] initWithRootViewController:loginController];
    }
